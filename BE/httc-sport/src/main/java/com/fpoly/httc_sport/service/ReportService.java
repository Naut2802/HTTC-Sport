...
public class ReportService {
	...
	public ReportResponse getReportByUserAndDate(String userId, LocalDate fromDate, LocalDate toDate, int page, int size) {
		Pageable pageable = PageRequest.of(page, size);
		var bills = billRepository.findByUserIdAndRentedAtBetween(userId, fromDate, toDate, pageable);
	...
	public void exportExcel(HttpServletResponse response, List<Long> billIds) throws IOException {
		...
		excelUtils.createRowHeader(rowHeader, 6, "Ngày đặt sân");
		excelUtils.createRowHeader(rowHeader, 7, "Thời gian bắt đầu");
		excelUtils.createRowHeader(rowHeader, 8, "Thời gian kết thúc");
		excelUtils.createRowHeader(rowHeader, 9, "Tổng tiền");
		excelUtils.createRowHeader(rowHeader, 10, "Loại sân đá");
		excelUtils.createRowHeader(rowHeader, 11, "Đánh giá");
		excelUtils.createRowHeader(rowHeader, 12, "Phương thức thanh toán");
		...
		bills.forEach(bill -> {
			...
			excelUtils.createCell(row, 6, bill.getRentedAt());
			excelUtils.createCell(row, 7, bill.getStartTime());
			excelUtils.createCell(row, 8, bill.getEndTime());
			excelUtils.createCell(row, 9, bill.getTotal());
			excelUtils.createCell(row, 10, bill.getTypePitch());
			excelUtils.createCell(row, 11, bill.isRate() ? "Đã đánh giá" : "Chưa đánh giá");
			excelUtils.createCell(row, 12, bill.getPaymentMethod());
		});
